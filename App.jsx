  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0D1B2A] rounded-3xl p-6 text-white shadow-xl">
        <p className="text-[10px] font-bold text-[#FFD700] tracking-widest uppercase mb-1">Totale Clienti</p>
      ...
      <div className="space-y-3">
        {shouldShowList ? (
          ...
          <div className="text-center py-12 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest px-10">Usa la ricerca per trovare un cliente</p>
          </div>
        )}
      </div>
  ...
  const handleStartScan = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        [span_4](start_span)// RegEx ottimizzata per estrarre solo: VIA/VIALE/CORSO + NOME + CIVICO/SNC[span_4](end_span)
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="font-bold text-[#0D1B2A] text-xs uppercase text-center">
          {selectedFile ? selectedFile.name : 'Carica documento PDF'}
        </p>
      </div>
      ...
      {scanResults.length > 0 && (
        <div className="space-y-4 pb-10">
          <h3 className="font-bold text-slate-400 text-[10px] uppercase tracking-widest text-left italic">Indirizzi trovati:</h3>
                ...
                {res.customer ? (
                  <div className="flex items-center gap-4 bg-yellow-50/50 p-4 rounded-2xl border-l-4 border-[#FFD700]">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-500 font-medium italic">Tocca per vedere i dettagli</p>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border-l-4 border-slate-300">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter italic">Non registrato</p>
                    </div>
...
function SistemaSection({ customers, refresh }) {
  const exportBackup = () => {
    ...
    link.download = `archivio_clienti.csv`;
    link.click();
  };
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          alert(`Importati correttamente ${cleanData.length} clienti!`);
          refresh();
        } catch (err) { alert("Errore nel caricamento del file CSV."); }
      }
    });
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-black text-[#0D1B2A] text-left">Impostazioni</h2>
      <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm text-center">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 italic">Clienti in archivio</p>
      ...
      <div className="grid gap-4">
        ...
        <label className="bg-[#0D1B2A] p-6 rounded-3xl text-white flex items-center justify-between shadow-xl cursor-pointer font-black uppercase text-sm text-left">
          <span>Importa Backup CSV</span>
          <Upload size={24} className="text-[#FFD700]" />
          <input type="file" accept=".csv" onChange={importBackup} className="hidden" />
        </label>
        
        <button onClick={() => { if(confirm("Sei sicuro? Questa operazione svuoterà l'archivio.")) db.customers.clear().then(refresh) }} className="text-red-400 text-[10px] font-bold uppercase mt-8 text-center w-full italic">
          Cancella database definitivamente
        </button>
      </div>
    </div>
  );
...
function CustomerDetailModal({ customer, onClose, onRefresh }) {
  ...
  const handleDelete = async () => {
    if(confirm(`Vuoi davvero eliminare ${customer.name}?`)) {
      ...
      <div className="bg-white w-full max-w-md rounded-t-[40px] p-8 shadow-2xl">
        ...
        {isEditing ? (
          <div className="space-y-4">
            ...
            <button onClick={handleUpdate} className="w-full bg-[#0D1B2A] text-white py-4 rounded-2xl font-black uppercase"><Save size={18} className="inline mr-2"/> Salva Modifiche</button>
          </div>
        ) : (
          <div className="space-y-6 text-left">
            <div>
              <h2 className="text-3xl font-black text-[#0D1B2A] uppercase leading-tight">{customer.name}</h2>
              <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest italic">Anagrafica Cliente</p>
            </div>
            <div className="space-y-4">
              ...
              {customer.phone && (
                <div className="flex items-center gap-4">
                  ...
                  <div><p className="text-[10px] font-bold text-slate-400 uppercase italic">Contatto telefonico</p><p className="font-bold">{customer.phone}</p></div>
                </div>
              )}
              {customer.instructions && (
                <div className="bg-yellow-50 p-5 rounded-3xl border border-yellow-100">
                  <p className="text-[10px] font-black text-[#FFD700] uppercase mb-1 italic">Istruzioni Scarico:</p>
    ...
    <div className="fixed inset-0 bg-white z-[100] p-6 pt-12 overflow-y-auto">
      <div className="max-w-md mx-auto text-left">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pb-10">
          ...
          <textarea name="instructions" className="w-full p-5 bg-slate-50 rounded-[25px] h-32" placeholder="ISTRUZIONI SCARICO (NOTE)..."></textarea>
          <button type="submit" className="w-full bg-[#FFD700] py-5 rounded-[30px] font-black uppercase shadow-lg">Registra in Archivio</button>
        </form>
      </div>
    </div>
  );
